...
// 		}
		...
		public static void  LoadAssetAsync<T>(string abPath, string assetName, Action<float, bool, T> OnLoadAsset) where T : UnityEngine.Object
		{
			//StartCoroutine(LoadAsset<T>(abPath, assetName, OnLoadAsset, true));
		}

		public static void LoadAssetSync<T>(string abPath, string assetName, Action<float, bool, T> OnLoadAsset) where T : UnityEngine.Object
		{
		...
		public static IEnumerator LoadAsset<T>(string abPath, string assetName, Action<float, bool, T> OnLoadAsset, bool isAsync = true) where T : UnityEngine.Object
		{
		...
		public static IEnumerator loadAllAssets<T>(string abPath, bool isAsync = true) where T : UnityEngine.Object
		{
			var bundle = AssetBundle.LoadFromFile(abPath);
			T[] objs = null;
			if (isAsync)
		...
		public static IEnumerator LoadFromManifest(string manifestPath, string abName, bool load, bool isAsync = true)
		{
			//加载那个打包时额外打出来的总包
			// 			StartCoroutine(
