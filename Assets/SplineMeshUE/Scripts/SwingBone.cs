...
public class SwingBone : MonoBehaviour
{
    ...
    public bool startParent = false;
    //public List<ColliderBone> colliders = new List<ColliderBone>();
    //当前节点下的子节点链.自动构造
    ...
    void Start()
    {
		...
		d.OnExit += OnLateUpdate;
    }

	Delay d = new Delay(0.5f);
    ...
    private void OnDrawGizmos()
    {
        if (isDebug)
        {
            ...
            for (int n = 0; n < nodes.Count; n++)
            {
                Gizmos.DrawWireSphere(nodes[n].curPos, radius);
            }
